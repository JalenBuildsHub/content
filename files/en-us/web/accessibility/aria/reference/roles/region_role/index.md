...
Do not add `role="region"` to an element solely because its content scrolls. The `region` role creates a navigational landmark and is appropriate only when the content is sufficiently important that users are likely to want to navigate to it directly.

If a scrollable area does qualify as a region, give it an accessible name. If a page contains more than one region, each should have a unique label.

Making a scroll container keyboard focusable, when needed, is separate from deciding whether it should be a landmark. Preserve the element's native semantics rather than replacing them with `region` unless the region semantics accurately describe the content.
